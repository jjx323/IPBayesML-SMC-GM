...
def cg_my(A, b, M=None, Minv=None, x0=None, tol=0.5, atol=0.1, maxiter=1000, 
    ...
    while k <= maxiter:
        if M is None and Minv is None:
            t1 = rk@rk
            if np.isposinf(t1): t1 = np.finfo(x0.dtype).max
            if np.isneginf(t1): t1 = np.finfo(x0.dtype).min
            betak = t1/(rk_@rk_)
            if np.isposinf(betak): betak = np.finfo(x0.dtype).max
            if np.isneginf(betak): betak = np.finfo(x0.dtype).min
            _pk = rk + betak*pk
        elif Minv is not None:
        ...
        w = np.squeeze(A*_pk)
        w[np.any(np.isposinf(w))] = np.finfo(x0.dtype).max
        w[np.any(np.isneginf(w))] = np.finfo(x0.dtype).min
        # w[np.any(np.isnan(w))] = np.finfo(x0.dtype).max
        _pk = _pk.squeeze()
        _pk[np.any(np.isposinf(_pk))] = np.finfo(x0.dtype).max
        _pk[np.any(np.isneginf(_pk))] = np.finfo(x0.dtype).min
        # _pk[np.any(np.isnan(_pk))] = np.finfo(x0.dtype).max
        assert ~np.any(np.isnan(_pk)), "_pk = NaN detected in cy_my"
        assert ~np.any(np.isnan(w)), "w = NaN detected in cy_my"
        t2 = _pk@w
        if np.isposinf(t2): t2 = np.finfo(x0.dtype).max
        if np.isneginf(t2): t2 = np.finfo(x0.dtype).min
        if np.isnan(t2): t2 = np.finfo(x0.dtype).max
    ...
    if info != 0 and info != "curvature_limit":
        # info = "reach_maxiter"
        info = maxiter
