...
def double_pass(H, M, Minv, n, r, omega=None, l=20, cutval=None, random_seed=None):
    ...
    if sps.isspmatrix(M):
        MM = M  ## ensure M not conflict with the name of the function M below
        def M(x):
            return np.array(MM@x)
    
    if sps.isspmatrix(Minv):
        MinvM = Minv  ## ensure Minv not conflict with the name of the function Minv below
        def Minv(x):
            return np.array(MinvM@x)
