...
results_folder = Path("results/example3SMCGM")
    ...
    def file_process(file_name, contents=None):
        ...
        if os.path.exists(file_name):
            ...
            try:
                ...
                print(f"Etem_true=true_fun.x.array[:]rror deleting '{file_name}': {e}")
...
############################### SMC ###########################################
num_particles = 60000  ## particle number
smc = SMC(comm, num_particles, model)
smc.prepare()
smc.gather_samples()
GMM=Mix_Gauss_IS_proposal(prior,comm=comm)

num_particles_smc2 = 100 ## used in smc2, which aimes to find h
idx_h = 0
h = 0.1  # at idx_h=0, the initial guess is 0.1
...
while sum_h < 1: 
    ...
    idx_h += 1
##################################################################################
    smc.resampling(model.loss_res,h)
    GMM.update_samples(smc.particles)
    if rank == 0: # write num of clusters
        with open(file_find_h, "a") as f:
            formatted_string = f", num of clusters ={len(GMM.weights):2d}\n"
            f.write(formatted_string)
...
np.save(results_folder / "idx_last_layer.npy", idx_h)
