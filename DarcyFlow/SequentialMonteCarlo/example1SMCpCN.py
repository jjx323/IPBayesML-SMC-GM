...
results_folder = Path("results/example1SMCpCN")
...
for idx_layer in range(100):
    ...
    ######## pCN for transition#####################################
    step,len_pCN=0.2,200
    for idx_particle in range(num_particles):
        u=particles[idx_particle]
        num_acc=0
        for idx_pCN in range(len_pCN):
            v= np.sqrt(1-step**2)*u+step*prior.generate_sample(1)[0]
            acc_rate=np.exp( phi(u)-phi(v))
            if np.random.uniform()<acc_rate:
                u=v
                num_acc+=1
            
            if idx_pCN>50 and idx_pCN%100==0: 
                #print(num_acc/idx_pCN)
                if num_acc/idx_pCN<0.15:
                    step*=0.9
                elif num_acc/idx_pCN>0.3:
                    step*=1.1
        particles[idx_particle]=u
